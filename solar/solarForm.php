...
function post_date(string $k): ?string
{
    $v = trim($_POST[$k] ?? '');
    return $v === '' ? null : $v;
}
...
$sol = [
    ...
    'servituds_detectades' => 0,
    'servituds_compatibles' => null,
    'servituds_detall' => '',
    'regim_cessio' => '',
    'finalitat_educativa_condicionada' => 0,
    'limit_temporal_text' => '',
    'reversio_text' => '',
    'info_publica_rbel' => 0,
    'data_info_publica' => '',
    'assabentat_consell_insular' => 0,
    'data_assabentat' => '',
    'qualificacio_urbanistica' => '',
    'parametres_edificatoris' => '',
    'usos_permesos' => '',
    'afectacio_apr' => 0,
    'condicions_higienicosanitaries_ok' => 0,
    'data_cert_higienico' => '',
    'detall_apr' => '',
    ...
    'serveis_basics_ok' => 0,
    'accessos_ok' => 0,
    'estudi_mobilitat_aportat' => 0,
    'accessibilitat_universal_ok' => 0,
    'detall_accessos' => '',
    'postobra_obra_nova_notari_feta' => 0,
    'data_obra_nova' => '',
    'postobra_inscripcio_registre_feta' => 0,
    'data_inscripcio' => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') !== 'delete') {
    ...
    $tipusSeleccionats = [];
    foreach (array_keys($checkedTipus) as $t)
        if (!empty($checkedTipus[$t]))
            $tipusSeleccionats[] = $t;
    if (!$tipusSeleccionats) {
        ...
        try {
            ...
            if ($hasExtended) {
                $allSrvCols = [
                    ...
                    'disponible' => ['i', null],
                    'cabal_m3h' => ['s', null],
                    'pressio_kgcm2' => ['s', null],
                    'grup_pressio_necessari' => ['i', null],
                    'kw_obra' => ['s', null],
                    'kw_funcionament' => ['s', null],
                    'requereix_ct' => ['i', null],
                    'cota' => ['s', null],
                    'diametre' => ['s', null],
                    'solucio_pluvials' => ['s', null],
                ];
                $srvCols = [];
                ...
                foreach ($serveis as $tipus => $v) {
                    ...
                    foreach ($srvCols as $c) {
                        if ($c === 'solar_id') {
                            ...
                            $t[] = 's';
                            $p[] = $cabal;
                        } elseif ($c === 'pressio_kgcm2') {
                            $t[] = 's';
                            $p[] = $pressio;
                        } elseif ($c === 'grup_pressio_necessari') {
                            $t[] = 'i';
                            $p[] = $grup;
                        } elseif ($c === 'kw_obra') {
                            $t[] = 's';
                            $p[] = $kw_obra;
                        } elseif ($c === 'kw_funcionament') {
                            $t[] = 's';
                            $p[] = $kw_func;
                        } elseif ($c === 'requereix_ct') {
                            $t[] = 'i';
                            $p[] = $req_ct;
                        } elseif ($c === 'cota') {
                            $t[] = 's';
            ...
            foreach ($docTypes as $tipus => $_lbl) {
                ...
                $data_ap = trim((string) ($row['data_aportacio'] ?? ''));
                $data_ap = ($data_ap === '') ? null : $data_ap;
                $obs = trim((string) ($row['observacions'] ?? ''));
                $obs = ($obs === '') ? null : $obs;
            ...
            foreach ($infTypes as $tipus => $_lbl) {
                ...
                $data_inf = trim((string) ($row['data_informe'] ?? ''));
...
function renderLinks(text, outEl) {
  ...
  outEl.innerHTML =
    ...
    uniq.map((u, index) => {
      ...
      return `<li><a href="${safe}" target="_blank" rel="noopener noreferrer" title="${safe}">Document ${index + 1}</a></li>`;
    }).join('') +
    '</ul>';
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  <style>
    .url-preview { font-size: 12px; margin-top: 4px; opacity: 0.9; }
        ...
        <table>
          <tr>
            ...
            <td colspan="2">
              ...
              <textarea id="obs_sollicitud" name="obs_sollicitud" class="contenedorFicha_Blanca" rows="7" cols="110"><?php echo h($exp['obs_sollicitud']); ?></textarea>
        <div class="url-preview" id="prev_obs_sollicitud"></div>
            </td>
          </tr>
          <tr>
            <td>
            ...
            <td colspan="2">
              ...
              <textarea id="obs_topografic" name="obs_topografic" class="contenedorFicha_Blanca" rows="7" cols="110"><?php echo h($exp['obs_topografic']); ?></textarea>
        <div class="url-preview" id="prev_obs_topografic"></div>
            </td>
          </tr>
          <tr>
            <td>
            ...
            <td colspan="2">
              ...
              <textarea id="obs_acord_ple" name="obs_acord_ple" class="contenedorFicha_Blanca" rows="7" cols="110"><?php echo h($exp['obs_acord_ple']); ?></textarea>
        <div class="url-preview" id="prev_obs_acord_ple"></div>
            </td>
          </tr>
        </table>
