...
<?php
	...
	$sql_municipis = $id_illa
		...
		: "SELECT id, nom FROM Municipi WHERE 1 = 0";
	...
	<head>
		...
		<script>
				...
				function carregarMunicipisPerIlla(illaId, municipiSeleccionat) {
					var municipiSelect = document.getElementById('id_municipi');
					if (!municipiSelect) return;

					municipiSelect.innerHTML = '<option value="">Selecciona un municipi</option>';
					if (!illaId) return;

					fetch('carregarMunicipis.php?illa=' + encodeURIComponent(illaId))
						.then(function(response) {
							return response.json();
						})
						.then(function(municipis) {
							municipis.forEach(function(municipi) {
								var option = document.createElement('option');
								option.value = municipi.id;
								option.textContent = municipi.nom;
								if (municipiSeleccionat && String(municipi.id) === String(municipiSeleccionat)) {
						...
						.catch(function() {
							// Si falla la càrrega, es manté només l'opció per defecte.
						});
				}

				document.addEventListener('DOMContentLoaded', function() {
					var illaSelect = document.getElementById('id_illa');
					if (!illaSelect) return;

					illaSelect.addEventListener('change', function() {
						carregarMunicipisPerIlla(this.value, '');
					});
				});
			</script>		
		<meta charset="UTF-8">
